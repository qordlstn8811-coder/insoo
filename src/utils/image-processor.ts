...
export async function processImageWithWatermark(
    ...
    const svgOverlay = `
      <svg width="${width}" height="${height}">
        ...
        <style>
          .title { fill: white; font-size: ${width * 0.045}px; font-weight: bold; filter: url(#shadow); opacity: 0.95; font-family: sans-serif; }
          .brand-bar { fill: rgba(0,0,0,0.5); }
          .brand-text { fill: #ffffff; font-size: ${width * 0.035}px; font-weight: bold; font-family: sans-serif; }
          .highlight { fill: #00FF00; }
        </style>
        
        <!-- Center Title with Shadow -->
        <text x="50%" y="50%" text-anchor="middle" class="title" dominant-baseline="middle">${text}</text>
        
        <!-- Bottom Branding Bar -->
        <rect x="0" y="${height * 0.85}" width="${width}" height="${height * 0.1}" class="brand-bar" />
        <text x="50%" y="${height * 0.91}" text-anchor="middle" class="brand-text">
    ...
    `;
        ...
        const pipeline = image
            .resize(800, 600, { fit: 'cover' }) // Standardize size
