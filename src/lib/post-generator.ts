    ...
    try {
        ...
        if (isInfoPost) {
            ...
            imageUrls = imagePrompts.map((p, index) => {
                // Thumbnail uses Pollinations with strict abstract/minimalist prompt
                const promptEnc = encodeURIComponent(`${p}, abstract, minimalist, 3d render, gradient, high quality, no text, no people, no realistic photo`);
